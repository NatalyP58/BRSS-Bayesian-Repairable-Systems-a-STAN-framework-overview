...
run_simulation_parallel_cluster <- function(m, k, param, time_trunc, SEED = NULL) {
  ...
  clusterExport(cl, varlist = c(
    "gen_failures_PR", "frequentist_estimation", "bayesian_estimation_gamma",
    "Lambda_PR", "lambda_PR", "log_like_PR", "modelo.stan.gamma", 
  ...
  single_run <- function(i) {
    ...
    # Generar datos
    time <- tryCatch({
      gen_failures_PR(k, c(beta_r, mu_T_r), time_trunc)
    }, error = function(e) {
