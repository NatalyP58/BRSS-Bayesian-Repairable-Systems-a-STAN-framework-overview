...
vehiculo_sel <- "A"  # cambiar el vehículo 

# Filtrar solo el vehículo seleccionado
data_veh <- data %>% filter(VEICULO == vehiculo_sel)

# Convertir la columna de inicio a fecha/hora
...
# Calcular tiempo de falla desde la primera falla del vehículo
data_veh <- data_veh %>%
  arrange(INICIO_PARADA2) %>%
  mutate(time_failure_hours =  as.numeric(difftime(INICIO_PARADA2, start_obs, units = "hours")))

# Sacar los 4 problemas más frecuentes dentro de ese vehículo
top4 <- data_veh %>%
  group_by(PROBLEMA) %>%
...
# Crear lista de tiempos por problema (cada vector inicia en t0=0)
...
run_application_parallel_cluster <- function(time , time_trunc, SEED = NULL) {
  ...
  clusterExport(cl, varlist = c(
    ...
    "Lambda_PR", "lambda_PR", "log_like_PR", 
  ...
  bayes_gamma <- tryCatch({
    bayesian_estimation_gamma(
      time, time_trunc,  beta_r = NULL, mu_T_r = NULL, 
  ...
  bayes_corr <- tryCatch({
    bayesian_estimation_corr(
      time, time_trunc,  beta_r = NULL, mu_T_r = NULL, 
      compiled_model = modelo.compilado.principal.corr , simulation = FALSE,
      seed = SEED 
    )
  }, error = function(e) {
...
calc_AIC_BIC <- function(results, times,  time_trunc) {
  ...
  # --- Frecuentista ---
  loglik_freq <- log_like_PR(results$freq_est, time=times,  time_trunc=time_trunc)
  AIC_freq <- 2 * k - 2 * loglik_freq
  BIC_freq <- k * log(n) - 2 * loglik_freq
  
  # --- Bayesiano (gamma) ---
  loglik_bayes <- log_like_PR(results$bayes_est_gamma, time=times,  time_trunc=time_trunc)
  AIC_bayes <- 2 * k - 2 * loglik_bayes
  BIC_bayes <- k * log(n) - 2 * loglik_bayes
  
  # --- Bayesiano (corr) ---
  loglik_bayes_corr <- log_like_PR(results$bayes_est_corr, time=times,  time_trunc=time_trunc)
...
calc_DIC <- function(fit_stan, times, time_trunc) {
  ...
  D_hat <- -2 * log_like_PR(par_mean, time=times,  time_trunc=time_trunc)
