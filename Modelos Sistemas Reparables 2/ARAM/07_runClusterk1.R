...
run_simulation_parallel_cluster <- function(m, k, param, time_trunc, SEED = NULL) {
  ...
  clusterExport(cl, varlist = c(
    "gen_failures_ARAM", "frequentist_estimation", "bayesian_estimation_gamma",
    "Lambda_ARAM", "lambda_ARAM", "log_like_ARAM", "modelo.stan.gamma", 
  ...
  single_run <- function(i) {
    ...
    # Generar datos
    time <- tryCatch({
      gen_failures_ARAM(k, c(beta_r, mu_T_r, theta_r), time_trunc)
    }, error = function(e) {
    ...
    bayes_gamma <- tryCatch({
      if (exists("modelo.compilado.gamma") && !is.null(modelo.compilado.gamma)) {
        bayesian_estimation_gamma(
          time, time_trunc, beta_r, mu_T_r, theta_r, 
