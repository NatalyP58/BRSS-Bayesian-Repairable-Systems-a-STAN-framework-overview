...
bayesian_estimation_corr <- function(time, time_trunc, beta_r = NULL, mu_T_r = NULL, compiled_model_corr = NULL, simulation = TRUE, seed = NULL) {
  # Si no se proporciona un modelo compilado, usar el global
  if (is.null(compiled_model_corr)) {
    if (!exists("modelo.compilado.corr")) {
      stop("modelo.compilado.corr no encontrado en el entorno global")
    }
  ...
  ajuste <- sampling(compiled_model_corr, data = datos.stan,
                     chains = 1, iter = 3000, warmup = 500,
                     control = list(adapt_delta = 0.9, max_treedepth = 15),
                     seed = if (is.null(seed)) 123L else as.integer(seed),
                     refresh = 100)
  
  zscores <- abs(geweke.diag(As.mcmc.list(ajuste))[[1]]$z[1:2])
  if (any(zscores > 1.96)) {
    return(list(success = FALSE, estimate = NULL, coverage = c(0, 0), fit = ajuste))
  }
