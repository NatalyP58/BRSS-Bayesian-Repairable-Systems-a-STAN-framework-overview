...
run_application_parallel_cluster <- function(time , time_trunc, SEED = NULL) {
  ...
  clusterExport(cl, varlist = c(
    ...
    "Lambda_PLP", "lambda_PLP", "log_like_PLP", 
...
calc_AIC_BIC <- function(results, times,  time_trunc) {
  ...
  # --- Frecuentista ---
  loglik_freq <- log_like_PLP(results$freq_est, time=times,  time_trunc=time_trunc)
  AIC_freq <- 2 * k - 2 * loglik_freq
  BIC_freq <- k * log(n) - 2 * loglik_freq
  
  # --- Bayesiano (gamma) ---
  loglik_bayes <- log_like_PLP(results$bayes_est_gamma, time=times,  time_trunc=time_trunc)
  AIC_bayes <- 2 * k - 2 * loglik_bayes
  BIC_bayes <- k * log(n) - 2 * loglik_bayes
  
  # # --- Bayesiano (corr) ---
  loglik_bayes_corr <- log_like_PLP(results$bayes_est_corr, time=times,  time_trunc=time_trunc)
...
calc_DIC <- function(fit_stan, times, time_trunc) {
  ...
  dev_samples <- apply(draws, 1, function(par){
    -2 * log_like_PLP(par, time=times,  time_trunc=time_trunc)
  })
  ...
  D_hat    <- -2 * log_like_PLP(par_mean, time=times,  time_trunc=time_trunc)
...
calc_DIC(results$ajuste_corr, times=times, time_trunc = time_trunc)
