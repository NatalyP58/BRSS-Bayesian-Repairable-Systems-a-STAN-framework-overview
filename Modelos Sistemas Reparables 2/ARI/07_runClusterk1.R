...
run_simulation_parallel_cluster <- function(m, k, param, time_trunc, SEED = NULL) {
  ...
  clusterExport(cl, varlist = c(
    "gen_failures_ARI", "frequentist_estimation", "bayesian_estimation_gamma",
    "Lambda_ARI", "lambda_ARI", "log_like_ARI", "modelo.stan.gamma", 
    "current_dir", "safe_temp_dir", "modelo.compilado.principal", "modelo.compilado.principal.corr", "seeds", "SEED"), 
    envir = environment())
  ...
  single_run <- function(i) {
    ...
    # Generar datos
    time <- tryCatch({
      gen_failures_ARI(k, c(beta_r, mu_T_r, rho_r), time_trunc)
    }, error = function(e) {
    ...
    bayes_gamma <- tryCatch({
      if (exists("modelo.compilado.gamma") && !is.null(modelo.compilado.gamma)) {
        bayesian_estimation_gamma(
          time, time_trunc, beta_r, mu_T_r, rho_r, 
    ...
    list(
      ...
      ajuste_gamma    = if (bayes_gamma$success ) bayes_gamma$fit else NULL,
...
tryCatch({
  ...
  # Guardar resultados
  saveRDS(results, file = file.path(current_dir, "resultadosk101_simulacion01.rds"))
