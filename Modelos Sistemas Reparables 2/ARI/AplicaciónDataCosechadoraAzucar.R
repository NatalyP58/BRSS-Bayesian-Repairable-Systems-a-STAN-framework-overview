...
ta   <- subset(subset(data, PROBLEMA=="Divisor de Linha"), VEICULO=="A")$y[-1]
...
run_application_parallel_cluster <- function(time , time_trunc, SEED = NULL) {
  ...
  clusterExport(cl, varlist = c(
    ...
    "Lambda_ARI", "lambda_ARI", "log_like_ARI", 
  ...
  bayes_gamma <- tryCatch({
    bayesian_estimation_gamma(
      time, time_trunc,  beta_r = NULL, mu_T_r = NULL, rho_r = NULL,
...
calc_AIC_BIC <- function(results, times, time_trunc) {
  ...
  # --- Bayesiano (gamma) ---
  loglik_bayes <- log_like_ARI(results$bayes_est_gamma, time=times, time_trunc=time_trunc)
  AIC_bayes <- 2 * k - 2 * loglik_bayes
  BIC_bayes <- k * log(n) - 2 * loglik_bayes
  
  # # # --- Bayesiano (corr) ---
  loglik_bayes_corr <- log_like_ARI(results$bayes_est_corr, time=times, time_trunc=time_trunc)
  AIC_bayes_corr    <- 2 * k - 2 * loglik_bayes
  BIC_bayes_corr    <- k * log(n) - 2 * loglik_bayes
