...
run_application_parallel_cluster <- function(time , time_trunc, tau, SEED = NULL) {
  ...
  clusterExport(cl, varlist = c(
    ...
    "Lambda_PIR", "lambda_PIR", "log_like_PIR", 
  ...
  bayes_gamma <- tryCatch({
    bayesian_estimation_gamma(
      time, time_trunc,  beta_r = NULL, mu_T_r = NULL, theta_r = NULL, tau,
...
times_offseason  # ver primeros valores
...
tryCatch({
  results <- run_application_parallel_cluster (
    ...
    #tau       = c(5478.9, 13861.3)
    tau        = c(5504.5167, 11652.7500), 
    SEED       = 20251025
  )
...
calc_AIC_BIC <- function(results, times, tau, time_trunc) {
 
  # Número de parámetros estimados
  k <- length(results$bayes_est_gamma) 
  
  # Número de observaciones (total de tiempos en todos los sistemas)
  n <- sum(sapply(times, length))
  
  # --- Frecuentista ---
  loglik_freq <- log_like_PIR(results$freq_est, times, tau, time_trunc)
  AIC_freq <- 2 * k - 2 * loglik_freq
  BIC_freq <- k * log(n) - 2 * loglik_freq
  
  # --- Bayesiano (gamma) ---
  loglik_bayes <- log_like_PIR(results$bayes_est_gamma, times, tau, time_trunc)
  AIC_bayes <- 2 * k - 2 * loglik_bayes
  BIC_bayes <- k * log(n) - 2 * loglik_bayes
  
  # # --- Bayesiano (corr) ---
  loglik_bayes_corr <- log_like_PIR(results$bayes_est_corr, times, tau, time_trunc)
...
ic_vals <- calc_AIC_BIC(results, times=times_offseason,  tau = c(5504.5167, 11652.7500), time_trunc = time_trunc )
print(ic_vals)
...
calc_DIC <- function(fit_stan, times, tau, time_trunc) {
  ...
  dev_samples <- apply(draws, 1, function(par){
    -2 * log_like_PIR(par, times, tau, time_trunc)
  })
  ...
  D_hat <- -2 * log_like_PIR(par_mean, times, tau, time_trunc)
...
calc_DIC(results$ajuste_gamma, times=times_offseason, tau = c(5504.5167, 11652.7500), time_trunc = time_trunc)

#calc_DIC(results$ajuste_corr, times=times, tau = c(10882.983), time_trunc = 22000)
